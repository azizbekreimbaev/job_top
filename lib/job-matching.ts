...
import type { AdzunaJob, JobMatch } from "../types/jobs.ts";
...
Use only facts present in the supplied profile and job snippet.
...
export function createJobMatchingRequest(
  ...
  return {
    ...
    input: `Score the job against the candidate facts between the delimiters.\n\n<candidate_profile>\n${JSON.stringify(createMatchingProfileFacts(profile))}\n</candidate_profile>\n\n<job_listing>\n${JSON.stringify({ title: job.title, company: job.company.display_name, location: job.location.display_name, descriptionSnippet: job.description, contractType: job.contract_type ?? null, contractTime: job.contract_time ?? null })}\n</job_listing>`,
    text: {
      format: {
        type: "json_schema",
        name: "job_match",
