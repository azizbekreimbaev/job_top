...
JobPilot is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile using GPT-5.6-luna. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
- Two options on upload:
  - "Extract from Resume" → GPT-5.6-luna parses resume and auto-fills profile form fields
  - "Skip" → resume stored as-is, profile unchanged
- User can manually edit any profile field at any time
- User can generate a clean professional PDF resume from their current profile data using GPT-5.6-luna
...
- GPT-5.6-luna scores each job 0-100 against user profile
...
- Match score section:
  ...
  - Match reason paragraph from GPT-5.6-luna
...
- GPT-5.6-luna synthesizes all extracted content into structured dossier
- Dossier displayed on job details page
- If company site cannot be found — GPT-5.6-luna generates best dossier from company name and job description alone
...
- Resume PDF upload with optional profile auto-fill via GPT-5.6-luna
- Resume PDF generation from profile data using GPT-5.6-luna
- Adzuna API job discovery — searches by title and location, category filtered to IT jobs
- GPT-5.6-luna job matching with score, reason, matched skills, missing skills
- Job details page with full structured description
- Company Research Agent — single Browserbase session browses company public pages, GPT-5.6-luna builds dossier
...
- GPT-5.6-luna match scores feel accurate and the reasoning makes sense
